...
# =============================================================================
# explain — הסבר מילולי: למה ההיגיון אומר לקנות / למכור / ניטרלי
# =============================================================================
def explain(res):
    """מחזיר (רשימת שורות הסבר [(emoji, text)], משפט שורה-תחתונה)."""
    p = res['parts']
    lines = []

    # 1. מגמה ראשית
    if p['מגמה (EMA)'] > 0:
        lines.append(("🟢", "המחיר נסחר **מעל הממוצעים הנעים** (50 ו-200) — מבנה של מגמה עולה."))
    elif p['מגמה (EMA)'] < 0:
        lines.append(("🔴", "המחיר **מתחת לממוצעים הנעים** — מבנה של מגמה יורדת."))
    else:
        lines.append(("⚪", "המחיר סביב הממוצעים הנעים — אין מגמה ברורה."))

    # 2. Supertrend
    if res['st_up']:
        lines.append(("🟢", f"ה-**Supertrend שורי** — המגמה הקצרה חיובית, עם תמיכה סביב {res['st_line']:.2f}."))
    else:
        lines.append(("🔴", f"ה-**Supertrend דובי** — המגמה הקצרה שלילית, עם התנגדות סביב {res['st_line']:.2f}."))

    # 3. MACD
    if p['MACD'] > 0:
        lines.append(("🟢", "ה-**MACD חיובי** — מומנטום המחיר תומך בעלייה."))
    else:
        lines.append(("🔴", "ה-**MACD שלילי** — המומנטום נחלש."))

    # 4. RSI
    rsi_v = res['rsi']
    if rsi_v > 70:
        lines.append(("🟠", f"ה-**RSI גבוה ({rsi_v:.0f})** — קניית יתר; סיכון לתיקון בטווח הקצר."))
    elif rsi_v < 30:
        lines.append(("🟢", f"ה-**RSI נמוך ({rsi_v:.0f})** — מכירת יתר; פוטנציאל לתיקון כלפי מעלה."))
    else:
        lines.append(("⚪", f"ה-**RSI ניטרלי ({rsi_v:.0f})** — ללא קיצוניות."))

    # 5. VWAP
    if p['VWAP'] > 0:
        lines.append(("🟢", "המחיר **מעל קו ה-VWAP** — הקונים בשליטה."))
    else:
        lines.append(("🔴", "המחיר **מתחת לקו ה-VWAP** — המוכרים בשליטה."))

    # 6. נפח
    if res['vol_ratio'] > 1.2:
        lines.append(("🟢", f"**נפח מסחר גבוה מהרגיל** (פי {res['vol_ratio']:.1f}) — אישור אמיתי לתנועה."))
    elif res['vol_ratio'] < 0.7:
        lines.append(("🟠", f"**נפח נמוך** (פי {res['vol_ratio']:.1f}) — עניין חלש, זהירות."))
    else:
        lines.append(("⚪", "נפח מסחר רגיל."))

    # 7. Squeeze
    if res['sq_on']:
        lines.append(("⚪", "**סחיטת תנודתיות (Squeeze)** — אנרגיה נצברת לקראת תנועה חדה; הכיוון עדיין לא הוכרע."))

    # 8. ADX — עוצמת המגמה (אמינות האיתות)
    if res['adx'] > 25:
        lines.append(("💪", f"**ADX {res['adx']:.0f}** — המגמה חזקה, ולכן האיתות אמין יותר."))
    else:
        lines.append(("〰️", f"**ADX {res['adx']:.0f}** — מגמה חלשה/דשדוש, ולכן האיתות פחות אמין."))

    # 9. מגמת השוק הכללית
    if res.get('regime_note'):
        lines.append(("🌍", res['regime_note']))

    # שורה תחתונה — ספירת פרמטרים
    bull = sum(1 for k, v in p.items() if v > 1)
    bear = sum(1 for k, v in p.items() if v < -1)
    if res['score'] >= config.SCORE_BUY:
        verdict = (f"**מסקנה: לשקול קנייה.** {bull} מתוך 7 הפרמטרים חיוביים מול {bear} שליליים, "
                   f"והציון המשוקלל {res['score']:.0f}/100 מצביע על עדיפות לקונים. "
                   f"הכניסה סביב {res['entry']:.2f}, עם סטופ ב-{res['stop']:.2f}.")
    elif res['score'] <= config.SCORE_SELL:
        verdict = (f"**מסקנה: חולשה / לשקול מכירה.** {bear} מתוך 7 הפרמטרים שליליים מול {bull} חיוביים, "
                   f"והציון {res['score']:.0f}/100 מצביע על עדיפות למוכרים. עדיף להימנע מכניסה כעת.")
    else:
        verdict = (f"**מסקנה: ניטרלי / המתנה.** הפרמטרים מעורבים ({bull} חיוביים, {bear} שליליים) "
                   f"והציון {res['score']:.0f}/100 קרוב למרכז — אין עדיפות ברורה לאף כיוון. "
                   f"עדיף להמתין לאיתות ברור יותר.")
